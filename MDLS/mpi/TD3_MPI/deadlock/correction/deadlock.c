...
void echange_dangereux(int rang, char *buf, char *buf2, int n) {
    MPI_Status sta;
    int vois = (rang+1) % 2;

    /* bloque pour n >= 262121 */
    MPI_Send(buf,  n, MPI_BYTE, vois, 0, MPI_COMM_WORLD);
...
int main(int argc, char **argv) {
    ...
    echange_dangereux(rang, buf, buf2, n);
    printf("Fin echange.\n");
