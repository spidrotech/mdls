...
void bords(double *x, struct info_t *info) {
    ...
    if (info->rang > 0) {
	MPI_Send(&(x[info->ideb]),   1, MPI_DOUBLE, info->rang-1, tag, MPI_COMM_WORLD);
	MPI_Recv(&(x[info->ideb-1]), 1, MPI_DOUBLE, info->rang-1, tag, MPI_COMM_WORLD, &sta);
    }

    if (info->rang < info->nproc-1) {
	MPI_Recv(&(x[info->ifin]),   1, MPI_DOUBLE, info->rang+1, tag, MPI_COMM_WORLD, &sta);
	MPI_Send(&(x[info->ifin-1]), 1, MPI_DOUBLE, info->rang+1, tag, MPI_COMM_WORLD);
    }
}
